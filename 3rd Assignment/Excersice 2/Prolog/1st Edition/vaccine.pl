...
checkthefile(Start,N,Stream,Result,Answer):-
  retractall(hashset(_,_)),
  read_line_s(Stream,_,A),
  %%atomics_to_string(A,AL),
  %%write(A),nl,
  bfs(A,Ans),
  retractall(hashset(_,_,_,_,_,_,_,_,_,_)),
  %%string_to_list(L,Ans),
  %%writeln(L),
  atom_chars(Str,Ans),
...
whilebfs(Line,[H|T],TheAnswer):-
  ...
  Q=T,
  %%writeln('The remaining stack is ' + Ans),
  %%writeln('The answer is ' + Ans),
  %%writeln([H|T]),
  %%This_is_not_atomic_term=(Remain,S),
  %%assertz(hashset(Remain,S)),
  %%term_string(This_is_not_atomic_term,Strings),
  %%string_to_atom(Strings,Hi),
  %%atom_string(Hi,Strings),
  %string_to_list(Remain,Remain),
  %string_to_list(Sstring,S),
  %%append(_,[FirstAn],Ans),
  (
    ...
    not(hashset(Remain,S,Bottom,Flag,Flag2,IsA,IsC,IsG,IsU,Compare))->
      ...
      %%writeln(H),
      %%SeenNew=Seen.put(Hi,1),
      %%writeln(SeenNew),
      %%string_to_list(Rm,Remain),
      %%string_to_list(Ss,S),
      %%string_to_list(Anss,Ans),
      %%writeln(Rm + Ss+ Anss+Line),
      Liner is Line+1,
      ...
      (
        ...
        (
          (Flag2*Flag=:=0)->
            ...
            (Flag=:=0)->
              (
              %%rnamatch(Remain,Reversed),
              %%push1(Q,Ans,'c',Reversed,Second1,1,Flag2,0,IsA,IsC,IsG,IsU,Q3),
              append(Ans,['c'],Copy1),
              noty(Compare,K),
              ...
              %%append(S,[Last],Snew),
              %%push1(Q3,Ans,'p',Remain1,Snew,0,0,0,IsA,IsC,IsG,IsU,Q4),

              %%append(_,[Checkprelast,Check],Snew),
              ,
...
,
                    ...
                    (
                      (Flag2*Flag=:=0)->(
                        ...
                        (Flag2=:=0)->(
                          %%reverse(Second1,Second1New),
                          %%push1(Q2,Ans,'r',Remain,Second1New,Flag,1,0,IsA,IsC,IsG,IsU,SosNewList),
                          append(Ans,['r'],Copy31),
